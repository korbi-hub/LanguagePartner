...
const openAiKey = "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";
...
async function generateAnswer(language, language_level, partner_name, human_messages, ai_messages) {
  // Create chat history
  const human_message_prompts = [];
  const input = human_messages[human_messages.length - 1];
  human_messages.pop();
  human_messages.forEach(message => {
    human_message_prompts.push(HumanMessagePromptTemplate.fromTemplate(message));
  });
  const ai_message_prompts = [];
  ai_messages.forEach(message => {
    ai_message_prompts.push(AIMessagePromptTemplate.fromTemplate(message));
  });
  const history_message = mergeArrays(human_message_prompts, ai_message_prompts);
...
async function getVocabularyFromSentence(input, output_language) {
  const chatPrompt = ChatPromptTemplate.fromPromptMessages([
    ...
    HumanMessagePromptTemplate.fromTemplate("Ich mag deutsch wegen der kultur. (output language english)"),
    AIMessagePromptTemplate.fromTemplate(`
      Ich,I
      mögen,to like
      deutsch,German
    ...
    HumanMessagePromptTemplate.fromTemplate("Hallo mir gehts gut, wie war dein Tag? (output language english)"),
    AIMessagePromptTemplate.fromTemplate(`
      Hallo,Hello
      gehen,to go
      gut,good
